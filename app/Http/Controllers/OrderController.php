...
class OrderController extends Controller
{
    ...
    public function detail($id) {
        ...
        if($order->starts < now() && $order->ends < now()){
        ...
        if($order->denda){
            $totalDenda = $order->harga *  $order->denda;
            $totalPembayaran = $order->harga + $totalDenda;
        }

        $now = Carbon::now();
        $tanggalSelesai = Carbon::parse($order->ends);
        $selisih = $now->diff($tanggalSelesai);
        ...
        $result = [
           ...
           'harga_awal' => $order->harga,
           'percent_denda' => $order->denda, 
        ];
    ...
    public function acc(Request $request, $paymentId) {
        ...
        foreach($orders as $orders) {
            Order::where('id', $orders)->update(['status' => 2]);
        }
    ...
    {
        // Logika perhitungan denda disini, sesuai dengan aturan yang Anda tentukan.
        // Misalnya, jika keterlambatan pengembalian, denda adalah 10% dari harga sewa.

        // Update denda pada data sewa yang sesuai
        $sewa = Order::find($request->input('id'));
        $sewa->denda = $sewa->harga * 0.10; // 10% dari harga sewa sebagai denda
        $sewa->save();

        return redirect()->route('order.index')->with('success', 'Denda berhasil dihitung dan diterapkan.');
    }
}
